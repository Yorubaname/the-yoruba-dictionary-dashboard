...
angular.module("NamesModule").controller("NameCounterController", [
  ...
  function($scope, api) {
    ...
    api.countNames("", function(resp) {
      $scope.count_all_names = resp.totalWords;
      ...
      $scope.count_published_names = resp.totalPublishedWords;
      ...
      $scope.count_unpublished_names = resp.totalNewWords;
      ...
      $scope.count_modified_names = resp.totalModifiedWords;
    ...
    api.getRecentFeedbacks(function(responseData) {
      responseData.slice(0, 4).forEach(function(n) {
        if (n !== undefined) $scope.feedbacks.push(n);
      });
    });
  }
]);
